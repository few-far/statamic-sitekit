...
@section('scripts')
	<script>
		document.addEventListener('cp:init', () => {
			Statamic.$components.register('redirect-listing', {
				template: `
					<data-list
						:rows="items"
						:columns="columns"
								...
								>
									<template slot="cell-source" slot-scope="{ row: redirect }">
										<div class="flex items-center">
											...
											<a :href="redirect.edit_url" class="font-mono text-2xs">@{{ redirect.source }}</a>
										</div>
									</template>
